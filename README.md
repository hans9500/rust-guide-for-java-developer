...
| 4 | 소유권 ⭐ | Move, Copy, Clone, RAII |
| 5 | 빌림 ⭐ | &T, &mut T, 빌림 규칙, Slice |
| 6 | 라이프타임 ⭐ | 'a, 댕글링 참조, 시나리오 10개 |
...
| 18 | 더 깊은 Rust | Drop, From/Into, Rc, RefCell, Cow, Pin, Deref |
| 19 | impl 완전 정복 ⭐ | impl 11가지 형태 — 고유/trait/제네릭/From/Bound/특화/라이프타임/const generics |
| 20 | 전체 예제 | 도서관 관리 시스템 (s1~s18 통합) |
| 21 | CLI 테트리스 🎮 | crossterm, 차등 렌더링, enum CellState, impl Write — 로컬 실행 전용 |
| 22 | 마무리 | 다음 단계 안내 |
...
> **s21 CLI 테트리스**는 `crossterm` 터미널 제어가 필요하므로 로컬에서만 실행 가능합니다.  
> `cargo run`으로 실행하세요.
